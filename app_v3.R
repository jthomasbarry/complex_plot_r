...
server <- function(input, output) {
  ...
  theta=seq(0,2*pi,by=0.005)
   ...
   output$cPlot <- renderPlot({
     ...
     title(main="Image Plot", xlab="Real", ylab="Imaginary")
     #plot axis lines
   ...
   output$jPlot <- renderPlot({
     ...
     title(main="Pre-image Plot (Cartesian)", xlab="Real", ylab="Imaginary")
     #plot axis lines
     lines(c(-100,100),c(0,0))
     lines(c(0,0),c(-100,100))
     #plot points on real axis
